...
bugliveonsiji=liveon.anggun.my.id
bugliveonloro=104.21.8.121
bugliveontelu=172.67.139.111

bugtselilmupedia=api.midtrans.com

bugtselilmupediasiji=ilmupedia.anggun.my.id
bugtselilmupedialoro=iped.anggun.my.id
...
trojanwslinkliveon="trojan://${uuid}@${bugliveonsiji}:443?path=%2Fanggun&security=tls&host=${domain}&type=ws&sni=${domain}#${user}"

trojanwslinkilped="trojan://${uuid}@${bugtselilmupediasiji}:443?path=%2Fanggun&security=tls&host=${domain}&type=ws&sni=${domain}#${user}"
...
echo -e "$COLOR1 ${NC} ${WH}Path        ${COLOR1}: ${WH}/anggun (/Custom path)"
echo -e "$COLOR1 ${NC} ${WH}Path WSS    ${COLOR1}: ${WH}wss://bug.com/anggun (/Custom path)" 
