...
####
echo "DOMAIN UTAMA ADALAH anggunre.shop"
echo "~~~~~~ petunjuk tentang custom subdomain ~~~~~~~~~"
echo "JIKA KAMU INPUT KATA: test ,maka hasilnya adalah test.anggunre.shop"
echo "JIKA KAMU INPUT KATA: custom ,maka hasilnya adalah custom.anggunre.shop"
echo "JIKA KAMU INPUT KATA: ainzoverlord ,maka hasilnya adalah ainzoverlord.anggunre.shop"
echo "JIKA KAMU INPUT KATA: sg3 ,maka hasilnya adalah sg3.anggunre.shop"
echo "JIKA KAMU INPUT KATA: memekpink ,maka hasilnya adalah memekpink.anggunre.shop"
echo ""
...
DOMAIN=anggunre.shop
SUB_DOMAIN=pro.${SUB}.${DOMAIN}
NS_DOMAIN=ns.${SUB_DOMAIN}
echo "DOMAIN kamu adalah : ${SUB_DOMAIN}"
sleep 3
echo "IP=${SUB_DOMAIN}" > /var/lib/anggun/ipvps.conf
set -euo pipefail
IP=$(wget -qO- icanhazip.com);
...
echo ""
    if [[ $host == "1" ]]; then
        ...
        if [ -z ${domainmu} ]; then
            ...
            echo "IP=$domainmu" > /var/lib/anggun/ipvps.conf
        fi
        clear
    elif [[ $host == "2" ]]; then
        subdomainsshweb
